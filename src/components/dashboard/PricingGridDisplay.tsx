  ...
  return (
    <Card className="p-6 mb-6 bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
      ...
      <div className="space-y-3">
        {displayGrid.generaliste != null && (
          <div className="flex justify-between text-sm">
            <span className="font-medium">Docteur Généraliste</span>
            <span>{formatPrice(displayGrid.generaliste)}</span>
          </div>
        )}
        {displayGrid.specialiste != null && (
          <div className="flex justify-between text-sm">
            <span className="font-medium">Docteur Spécialiste</span>
            <span>{formatPrice(displayGrid.specialiste)}</span>
          </div>
        )}
        {displayGrid.professeur != null && (
          <div className="flex justify-between text-sm">
            <span className="font-medium">Professeur</span>
            <span>{formatPrice(displayGrid.professeur)}</span>
          </div>
        )}
      </div>
    </Card>
